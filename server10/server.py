...
class TCPMonitoringHandler(SimpleHTTPRequestHandler):
    ...
    def __init__(self, *args, config: Config, rate_limiter: RateLimiter, server_start_time: float, **kwargs):
        self.config = config
    ...
    def _handle_static(self):
        ...
        try:
            ...
            path = self.path.split('?', 1)[0]  # Remove query parameters
            path = path.split('#', 1)[0]  # Remove fragments
            ...
            if full_path.suffix and not full_path.suffix in self.config.allowed_extensions:
                return self._handle_error(403, "File type not allowed", f"Extension {full_path.suffix} not permitted")
            
            # Check if file exists
            if not full_path.is_file():
                return self._handle_error(404, "File not found", f"Requested path: {self.path}")
            
            # Serve the file using the parent class implementation
...
async def websocket_handler(websocket, path, config: Config, ws_rate_limiter: RateLimiter):
...
async def run_server():
    ...
    try:
        ...
        ws_server = await websockets.serve(
            lambda ws, path: websocket_handler(ws, path, config, ws_rate_limiter),
            config.host, 
            config.ws_port, 
            ping_interval=20, 
            ping_timeout=60 
